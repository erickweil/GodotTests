...
void storeV3(vec3 v, uint index) {
	ivec2 id = ivec2(index % _params.texWidth, index / _params.texWidth);
	imageStore(_output_tex, id, vec4(v.x,v.y,v.z,1));
}
...
void addQuad(vec3 pos0, vec3 pos1, vec3 pos2, vec3 pos3) {
...
float calcDe(vec3 p) {
	return sdTorus(p - vec3(_params.offx,_params.offy,_params.offz), vec2(0.33,0.13));
}
...
void main() {
	...
	// only output triangles if center dist is less than zero
	if(dist <= 0) {
